// Función auxiliar para calcular fechas relativas a Pentecostés
function calcularFechasMoviles(year, fechaPentecostesStr) {
    // Espera formato "DD/MM", por ejemplo "24/5"
    const [day, month] = fechaPentecostesStr.split('/').map(Number);
    
    // Crear el objeto de fecha nativo (Mes es 0-indexed en JS)
    const pentecostes = new Date(year, month - 1, day);
    
    // Función interna para sumar días y devolver formato "D/M"
    ...
    return {
        santisimaTrinidad: sumarDias(7),   // 1er Domingo después de Pentecostés
        corpusChristi: sumarDias(11),      // Jueves después de la Santísima Trinidad
        sagradoCorazon: sumarDias(19),     // Viernes después del 2do Domingo de Pentecostés
        inmaculadoCorazon: sumarDias(20)   // Sábado siguiente al Sagrado Corazón
    };
}

// Función principal para obtener el catálogo completo del año
function obtenerCatalogoSanto(year, fechaPentecostes, fechaMiercolesCeniza) {
    
    // 1. Calculamos las posiciones de las fechas móviles de este año
    const moviles = calcularFechasMoviles(year, fechaPentecostes);

    // 2. Retornamos el catálogo estructurado
    const catalogo = {
        // Fechas Especiales Variables (Ceniza)
        [fechaMiercolesCeniza]: [{ nombre: "Miércoles de Ceniza", enlace: "/tiempo/cuaresma/ceniza/miercoles"}],
        ...
        "28/2": [{ node: "San Policarpo, Obispo Y Mártir", enlace: "/tiempo/santos/febrero/23-SAN POLICARPO, obispo y mártir" }],
        ...
        "25/4": [
            ...
            { nombre: "San Marcos, Evangelista", enlace: "/tiempo/santos/abril/25-SAN MARCOS, Evangelista" }
        ],
        ...
        "20/5": [{ nombre: "María, Madre De La Iglesia", enlace: "/tiempo/santos/mayo/20-MARÍA, MADRE DE LA IGNACIA" }],
        "26/5": [
        ...
        "24/7": [{ node: "San Francisco Solano, Presbítero", enlace: "/tiempo/santos/julio/24-SAN FRANCISCO SOLANO, presbítero" }],
    ...
    // 3. Inyección de solemnidades dinámicas basadas en Pentecostés
    // Santísima Trinidad
    catalogo[moviles.santisimaTrinidad] = [{ nombre: "La Santísima Trinidad", enlace: "/tiempo/santos/junio/07-LA SANTÍSIMA TRINIDAD" }];
    
    // Corpus Christi
    catalogo[moviles.corpusChristi] = [{ nombre: "El Santísimo Cuerpo Y Sangre De Cristo (Corpus Christi)", enlace: "/tiempo/santos/junio/14-EL SANTÍSIMO CUERPO Y SANGRE DE CRISTO" }];
    
    // Sagrado Corazón de Jesús
    catalogo[moviles.sagradoCorazon] = [{ nombre: "El Sagrado Corazón De Jesús", enlace: "/tiempo/santos/junio/19-EL SAGRADO CORAZÓN DE JESÚS" }];

    // Inmaculado Corazón de María
    catalogo[moviles.inmaculadoCorazon] = [{ nombre: "El Inmaculado Corazón De La Santísima Virgen María", enlace: "/tiempo/santos/junio/20-EL INMACULADO CORAZÓN DE LA SANTÍSIMA VIRGEN MARÍA" }];
